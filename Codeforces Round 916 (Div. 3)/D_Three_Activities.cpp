...
void solve(int n, vll &a, vll &b, vll &c)
{
    vvll aa(n), bb(n), cc(n);
    lp(i, 0, n)
        aa[i] = {a[i], i};
    ...
    sort(rall(bb));
    sort(rall(cc));
    ll mxa[3] = {}, mxb[3] = {}, mxc[3] = {};

    mxa[0] = aa[0][1], mxa[1] = aa[1][1], mxa[2] = aa[2][1];
    mxb[0] = bb[0][1], mxb[1] = bb[1][1], mxb[2] = bb[2][1];
    mxc[0] = cc[0][1], mxc[1] = cc[1][1], mxc[2] = cc[2][1];
    ...
    lp(i, 0, 3)
    {
        lp(j, 0, 3)
        {
            if(mxb[j] == mxa[i]) continue;
            lp(k, 0, 3)
            {
                if(mxc[k] == mxb[j] || mxc[k] == mxa[i]) continue;
                ans = max(ans, a[mxa[i]] + b[mxb[j]] + c[mxc[k]]);
            }
        }
    }
    cout<<ans<<endl;
