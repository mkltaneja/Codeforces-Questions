...
{
    vi numDist(k+1, -1), dist(n+1, 0);
    
    int ans = INT_MAX;
    vector<pair<int,int>> nmx(k+1, {0, 0});
    vi nsum(k+1, 0);
    
    for(int i = 0; i < n; i++)
    {
        dist[i] = i - numDist[a[i]] - 1;
        numDist[a[i]] = i;
        ...
        else nmx[a[i]].s = max(nmx[a[i]].s, dist[i]);
    }
    ...
    lp(i, 1, k+1)
    {
        ...
        else nmx[i].s = max(nmx[i].s, d);

        ans = min(ans, max(nmx[i].f/2, nmx[i].s));
    }
