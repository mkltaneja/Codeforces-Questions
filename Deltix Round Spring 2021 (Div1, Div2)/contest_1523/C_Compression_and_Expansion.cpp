...
bool dfs(int i, int n, vi &a, string x, vs &ans)
{
    if(i == n) 
    {
        ans.push_back(x);
    ...
    {
        ans.push_back(x);
        if(dfs(i+1, n, a, x+".1", ans)) return true;
        ans.pop_back();
    }
    else 
    {
        int num = 0, k = 0, kans = -1;
        ...
        if(kans == 0) 
        {
            if(dfs(i+1, n, a, x.substr(0,kans)+to_string(a[i]), ans)) return true;
        }
        else 
        {
            if(dfs(i+1, n, a, x.substr(0,kans)+"."+to_string(a[i]), ans)) return true;
