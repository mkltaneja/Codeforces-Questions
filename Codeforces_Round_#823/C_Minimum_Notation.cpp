...
void solve(int n, string &a)
{
    ...
    {
        if(st2.count({p.s, 1}) || st2.count({p.s, 0})) continue;
    
        st2.insert({p.s, 0});

        lpr(i, p.s-1, -1)
        {
            if(st2.count({i, 0}) || st2.count({i, 1})) break;
    ...
    for(auto p : st2)
    {
        if(p.s) ans.push_back(min(9, (a[p.f]-'0')+1));
        else ans.push_back(a[p.f]-'0');
    }
    sort(all(ans));
