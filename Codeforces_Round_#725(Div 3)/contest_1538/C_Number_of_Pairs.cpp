...
void solve1(int n, vi &a, int l, int r)
{
    sort(all(a));
    ull ans = 0;
    for(int i = n-1; i >= 0; i--)
    {
        ...
        if(a[i] >= l)
        {
            auto itr = lower_bound(all(a), r-a[i]+1);
            if(itr - a.begin() >= i) 
                ans += i;
            else ans += (itr - a.begin());
        }
        else 
        {
            auto itr1 = lower_bound(all(a), l-a[i]);
            auto itr2 = lower_bound(all(a), r-a[i]+1);
            if(itr1 - a.begin() >= i) continue;
            else if(itr2 - a.begin() >= i) ans += i - (itr1 - a.begin());
            else ans += (itr2 - itr1);
        }
    }
    cout<<ans<<"\n";
...
void solve2(int n, vi &a, int l, int r)
{
    sort(all(a));
    ull ans = 0;
    lp(i, 0, n)
    {
        auto i1 = upper_bound(all(a), r-a[i]) - a.begin();
        auto i2 = lower_bound(all(a), l-a[i]) - a.begin();
        ans += i1 - i2;
