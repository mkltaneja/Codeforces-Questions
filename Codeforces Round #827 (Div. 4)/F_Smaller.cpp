...
bool check(ll l1, ll l2, vll &mps, vll &mpt)
{
    int mx = -1;
    lp(i, 0, 26)
    {
    ...
    {
        // cout<<mps[i]<<", "<<mpt[i]<<";  ";
        if(win == 1 && mpt[i]) return true;
        else if(win == 2 && mps[i]) return false;
        if(!win && mps[i] > mpt[i]) win = 1;
        else if(!win && mpt[i] > mps[i]) win = 2;
    }
    // cout<<endl;
