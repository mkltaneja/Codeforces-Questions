...
vi dp(1001, INT_MAX);
vi mn;
ll knapsack(int k, vi &mn, vi &c, int n)
{
    vector<int> dp(k + 1, 0);

    for (int i = 1; i <= n; i++)
        for (int j = k; j >= mn[i-1]; j--)
            dp[j] = max(c[i - 1] + dp[j - mn[i - 1]], dp[j]);
    return dp[k];
}

void solve(int n, int k, vi &b, vi &c)
{
    cout << knapsack(k, mn, c, n) << endl;
}
