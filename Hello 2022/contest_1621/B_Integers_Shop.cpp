...
{
    int minl = INT_MAX, maxr = INT_MIN, minlc = INT_MAX, maxrc = INT_MAX, maxlen = INT_MIN, maxlenc = INT_MAX, ans = INT_MAX;
    for(vector<int> v : a)
    {
        int l = v[0], r = v[1], c = v[2];
        
        if(l < minl) minl = l, minlc = c;
        else if(l == minl) minlc = min(minlc, c);
        if(r > maxr) maxr = r, maxrc = c;
        else if(r == maxr) maxrc = min(maxrc, c);

        if(r-l+1 > maxlen) maxlen = r-l+1, maxlenc = c;
        else if(r-l+1 == maxlen) maxlenc = min(maxlenc, c);

        ans = minlc + maxrc;
        if(maxlen == maxr - minl + 1) ans = min(ans, maxlenc);
        cout<<ans<<endl;
    }
}
